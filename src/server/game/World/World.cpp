...
void World::LoadConfigSettings(bool reload)
{
    ...
    {
        TC_LOG_ERROR("server.loading", "Anticheat.Movement.TeleportToPlaneAlarms (%d) must be >= 20. Using 20 instead.", m_TeleportToPlaneAlarms);
        m_TeleportToPlaneAlarms = 20;
    }
    if (m_TeleportToPlaneAlarms > 100)
    {
        TC_LOG_ERROR("server.loading", "Anticheat.Movement.TeleportToPlaneAlarms (%d) must be <= 100. Using 100 instead.", m_TeleportToPlaneAlarms);
    ...
    {
        TC_LOG_ERROR("server.loading", "Anticheat.Movement.m_MistimingDelta (%d) must be >= 5000ms. Using 5000ms instead.", m_MistimingDelta);
        m_MistimingDelta = 5000;
    }
    if (m_MistimingDelta > 50000)
    {
        TC_LOG_ERROR("server.loading", "Anticheat.Movement.m_MistimingDelta (%d) must be <= 50000ms. Using 50000ms instead.", m_MistimingDelta);
    ...
    {
        TC_LOG_ERROR("server.loading", "Anticheat.Movement.MistimingAlarms (%d) must be >= 100. Using 100 instead.", m_MistimingAlarms);
        m_MistimingAlarms = 100;
    }
    if (m_MistimingAlarms > 500)
    {
        TC_LOG_ERROR("server.loading", "Anticheat.Movement.m_MistimingAlarms (%d) must be <= 500. Using 500 instead.", m_MistimingAlarms);
    ...
    m_int_configs[CONFIG_CHAT_STRICT_LINK_CHECKING_CUSTOM_LINKS] = sConfigMgr->GetIntDefault("ChatStrictLinkChecking.CustomLinks", 0);
