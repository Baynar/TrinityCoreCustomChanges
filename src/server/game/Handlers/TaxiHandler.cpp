...
void WorldSession::SendTaxiStatus(ObjectGuid guid)
{
    ...
    // find taxi node
    uint32 nearest = sObjectMgr->GetNearestTaxiNode(unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ(), unit->GetMapId(), player->GetTeam(), 0);
    if (!nearest)
        return;
...
void WorldSession::HandleMoveSplineDoneOpcode(WorldPacket& recvData)
{
    ...
    const Unit *mover = _player->m_playerMovingMe;
    ...
    if (curDest)
    {
        ...
        if (curDestNode && curDestNode->map_id == GetPlayer()->GetMapId())
            while (GetPlayer()->GetMotionMaster()->GetCurrentMovementGeneratorType() == FLIGHT_MOTION_TYPE)
                //GetPlayer()->GetMotionMaster()->Move;
