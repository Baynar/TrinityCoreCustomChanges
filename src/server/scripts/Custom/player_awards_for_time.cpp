	...
	void OnTime(Player* player, uint32 total, uint32 /*level*/)
	{
		...
		switch (total)
		{
			case 7200: // 2 hours
				...
				ChatHandler(player->GetSession()).SendSysMessage("РЎРѕРѕР±С‰РµРЅРёРµ");
				break;
			case 14400: // 4 hours
			...
			case 28800: // 8 hours
				...
				break;
			case 57600: // 16 hours
				// РњР°РєСЃРёРјСѓРј 12 РІРµС‰РµР№!
				// mailItems.push_back(ItemPair(РРґ РІРµС‰Рё, РєРѕР»-РІРѕ));
				mailItems.push_back(ItemPair(5133, 3));
				SendMail(player, "Р—Р°РіРѕР»РѕРІРѕРє РїРёСЃСЊРјР°", "РЎРѕРґРµСЂР¶Р°РЅРёРµ РїРёСЃСЊРјР°");
				ChatHandler(player->GetSession()).SendSysMessage("РЎРѕРѕР±С‰РµРЅРёРµ");
				break;
			case 115200: // 32 hours
				...
				SendMail(player, "Р—Р°РіРѕР»РѕРІРѕРє РїРёСЃСЊРјР°", "РЎРѕРґРµСЂР¶Р°РЅРёРµ РїРёСЃСЊРјР°", 5000000); // Send Items +500 gold
				ChatHandler(player->GetSession()).SendSysMessage("РЎРѕРѕР±С‰РµРЅРёРµ");
				break;
			default:
				break;
