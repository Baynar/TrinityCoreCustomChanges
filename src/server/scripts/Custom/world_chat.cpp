...
/* CONFIG VARIABLES */
int config_ChatEnabled = sWorld->getBoolConfig(CONFIG_WORLD_CHAT_ENABLED);
int config_ChatCrossFactions = sWorld->getBoolConfig(CONFIG_WORLD_CHAT_CROSSFACTION);

/* STRUCTURE FOR WorldChat map */
struct ChatElements
{
	uint8 chat = 1; // CHAT DISABLED BY DEFAULT
};
		...
		for (SessionMap::iterator itr = sessions.begin(); itr != sessions.end(); ++itr)
		{
			...
			if (WorldChat[guid2].chat == 1) // Checks if player have the World Chat visible
			{
				if (config_ChatCrossFactions) // Checks if the chat is Cross Factions
				{
					...
					{
						snprintf(message, 1024, "[World][%s][%s%s|r]: %s%s|r", world_chat_GMIcon.c_str(), world_chat_ClassColor[player->getClass() - 1].c_str(), pChat->GetNameLink(player).c_str(), WORLD_CHAT_WHITE.c_str(), msg);
					}
					else
						snprintf(message, 1024, "[World][%s][%s%s|r]: %s%s|r", world_chat_TeamIcon[player->GetTeamId()].c_str(), world_chat_ClassColor[player->getClass() - 1].c_str(), pChat->GetNameLink(player).c_str(), WORLD_CHAT_WHITE.c_str(), msg);
